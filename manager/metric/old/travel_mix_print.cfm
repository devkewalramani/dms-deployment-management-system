...
from dbo.schedule
Where (Startdate between '#form.startdate#' and '#form.enddate#') and (ProjectType ='EFD' or ProjectType ='EFD/LOE') and (In_Region='Y') and (Status <> 'Canceled')
</cfquery>

<cfquery name="total_out" datasource="SBSEFD">
select count(*) as total_outregion
from dbo.schedule
Where (Startdate between '#form.startdate#' and '#form.enddate#') and (ProjectType ='EFD' or ProjectType ='EFD/LOE') and (In_Region='N') and (Status <> 'Canceled')
</cfquery>

<cfquery name="total_international" datasource="SBSEFD">
select count(*) as total_inter
from dbo.schedule
Where (Startdate between '#form.startdate#' and '#form.enddate#') and (ProjectType ='EFD' or ProjectType ='EFD/LOE') and (ProjectState='International') and (Status <> 'Canceled')
</cfquery>
...
<cfset p_drive=#drive#/#total.totaljobs#>
...
<cfchart font="Arial" showXGridlines="yes" showYGridlines="yes" show3d="yes" labelformat="percent" chartheight="400" chartwidth="600" title="SBS Travel Mix from #form.startdate# to #form.enddate#" format="jpg"> 
