...
<!---search database based on search critera ---> 

<cfif #Form.searchcode# eq 1> <!---Keyword Search--->
	
	<cfset SearchKeyword = '#Form.search#'&'%'>
	
	<cfquery name="search" datasource="SBSEFD">
	SELECT *
	FROM dbo.schedule
	WHERE ProjectName LIKE '#SearchKeyword#'
	</cfquery>

<cfelseif #Form.searchcode# eq 2><!---Specfic Search--->

	<cfif #Form.ProjectType# eq 0 and #Form.Status# neq 0>

	<cfquery name="search" datasource="SBSEFD">
	SELECT *
	FROM dbo.schedule
	WHERE (dispatchid = '#form.dispatchid#') and (Status = '#Form.Status#')
	</cfquery>

	<cfelseif #Form.ProjectType# eq 0 and #Form.Status# eq 0>
	
	<cfquery name="search" datasource="SBSEFD">
	SELECT *
	FROM dbo.schedule
	WHERE (dispatchid = '#form.dispatchid#')
	</cfquery>
	
	<cfelseif #Form.ProjectType# neq 0 and #Form.Status# eq 0>
	
	<cfquery name="search" datasource="SBSEFD">
	SELECT *
	FROM dbo.schedule
	WHERE (dispatchid = '#form.dispatchid#') and (ProjectType = '#Form.ProjectType#')
	</cfquery>
	...
	SELECT *
	FROM dbo.schedule
	WHERE (dispatchid = '#form.dispatchid#') and (Status = '#Form.Status#') and (ProjectType = '#Form.ProjectType#')
	</cfquery>

	</cfif>
</cfif>

<span class="pageheader"> Please select a project </span>
<br>
<div style="height: 400px; width: 616; overflow: auto">
<table width="100%" bgcolor="#CCCCCC">
  <tr>
    <th bgcolor="#FFFFFF" class="style12" scope="col">Dexterra ID</th>
    <th bgcolor="#FFFFFF" class="style12" scope="col">Project Name </th>
	<th bgcolor="#FFFFFF" class="style12" scope="col">Engineer</th>
    <th bgcolor="#FFFFFF" class="style12" scope="col">Start Date</th>
	<th bgcolor="#FFFFFF" class="style12" scope="col"></th>
    <th bgcolor="#FFFFFF" class="style12" scope="col"></th>
  </tr>
  <!--- output results, the options displayed depends on the current status --->
<cfoutput query="search">
<form name="form"  method="post" action="options.cfm" target="_blank">
<input name="rid" type="hidden" value="#search.RID#" size="4" maxlength="4">
  <tr>
    <td bgcolor="##FFFFFF" class="style12">#search.DispatchID#	</td>
    <td bgcolor="##FFFFFF" class="style12">#search.ProjectName#</td>
	<td bgcolor="##FFFFFF" class="style12">#search.SE_Lname#,#search.SE_Fname#</td>
    <td bgcolor="##FFFFFF" class="style12">#DateFormat(search.StartDate,'mm/dd/yyyy')#</td>
	<td bgcolor="##FFFFFF" class="style12">
	<cfif #search.status# eq 'Scheduled'>
	<select name="option">
	...
	<cfelseif #search.status# eq 'Completed'>
	<select name="option">
    <option value="0">View</option>
	<option value="1">Edit</option>
	<option value="3">Complete</option>
    </select>
	<cfelseif #search.status# eq 'Canceled'>
	<select name="option">
    <option value="0">View</option>
    <option value="2">Reschedule</option>
    </select> 
	<cfelseif #search.status# eq 'Vacation'>
	<select name="option">
    <option value="0">View</option>
    <option value="1">Edit</option>
    <option value="4">Cancel</option>
	...
	<td bgcolor="##FFFFFF"><input name="Submit" type="submit" value="Select" class="style12"></td>
    </tr>
</form>
</cfoutput>
</table>
